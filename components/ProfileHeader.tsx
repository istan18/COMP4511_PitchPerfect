...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({
  ...
  return (
    <View className="items-center">
      <View
        className={`${otherStyles} mt-12 flex w-9/10 items-center justify-around flex-row gap-x-8`}
      >
        <Image source={imageSource} className="rounded-full h-40 w-40" />

        <View className="flex flex-col gap-y-2 mx-auto">
          <Text className="mr-auto text-left text-white text-3xl mb-2">
            {name}
          </Text>
          ...
          <View className="flex flex-row items-center gap-x-4">
            <Text className="text-white text-xl text-left">{university}</Text>
      ...
      <View
        className={
          "flex flex-row justify-between items-center w-9/10 mt-4 gap-x-4"
        }
      >
        {buttons &&
