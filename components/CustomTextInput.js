  ...
  return (
    <react_1.default.Fragment>
      <react_native_1.View className={`${width} mx-auto ${marginBottom}`}>
        {heading ? (
          <react_native_1.Text
            className={"text-white text-3xl mx-auto mb-3 ml-0"}
          >
            {heading}
          </react_native_1.Text>
        ) : null}
        ...
        />
        {includeCounter && (
          <react_native_1.Text className={"text-white text-lg ml-auto"}>
            {text.length}/{maxLength}
          </react_native_1.Text>
        )}
        <react_native_1.View className={"absolute left-3 top-3"}>
          {leftIcon}
        </react_native_1.View>
        <react_native_1.View className={"absolute top-3 right-3"}>
          {rightIcon}
        </react_native_1.View>
        {children}
      </react_native_1.View>
